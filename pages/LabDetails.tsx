...
const LabDetails = () => {
    ...
    const handleDownloadSafetyReport = () => {
        const content = `LAPORAN KESELAMATAN - ${lab.name}\nTanggal: ${new Date().toLocaleDateString('id-ID')}\nLokasi: ${lab.location}\nManajer: ${lab.manager || '-'}\n\nLevel Keselamatan: ${lab.safetyLevel || '-'}\nKapasitas: ${lab.capacity || '-'} Mahasiswa\nStatus: Operasional\n\n-- Akhir Laporan --`;
                    ...
                    <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between text-sm border-b border-gray-100 pb-2">
                                ...
                                <span className="text-gray-900 font-medium">{lab.manager || '-'}</span>
                            </div>
                            <div className="flex justify-between text-sm border-b border-gray-100 pb-2">
                                <span className="text-gray-500">Kapasitas</span>
                                <span className="text-gray-900 font-medium">{lab.capacity ? `${lab.capacity} Mahasiswa` : '-'}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-500">Level Keselamatan</span>
                                <span className="text-gray-900 font-medium">{lab.safetyLevel || '-'}</span>
                            </div>
                        </div>
                    </div>
