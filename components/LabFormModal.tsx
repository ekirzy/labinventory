  ...
  useEffect(() => {
    if (lab) {
      setFormData({
        ...
        safetyLevel: lab.safetyLevel || 'Rendah'
      });
    }
  }, [lab, isOpen]);
        ...
        <div className="p-8 overflow-y-auto">
          <form id="lab-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <label className="flex flex-col gap-1">
                <span className="text-sm font-medium text-gray-700">Manajer Lab</span>
                <input value={formData.manager} onChange={e => setFormData({ ...formData, manager: e.target.value })} className="bg-white border border-gray-300 rounded-lg h-11 px-3 text-gray-900 focus:ring-2 focus:ring-primary/50 focus:border-primary outline-none" placeholder="Nama Manajer" />
              </label>
              <label className="flex flex-col gap-1">
                <span className="text-sm font-medium text-gray-700">Kapasitas (Orang)</span>
                <input type="number" min="0" value={formData.capacity} onChange={e => setFormData({ ...formData, capacity: parseInt(e.target.value) || 0 })} className="bg-white border border-gray-300 rounded-lg h-11 px-3 text-gray-900 focus:ring-2 focus:ring-primary/50 focus:border-primary outline-none" />
              </label>
            </div>

            <label className="flex flex-col gap-1">
              <span className="text-sm font-medium text-gray-700">Level Keselamatan</span>
              <div className="relative">
                <select value={formData.safetyLevel} onChange={e => setFormData({ ...formData, safetyLevel: e.target.value as any })} className="appearance-none w-full bg-white border border-gray-300 rounded-lg h-11 px-3 text-gray-900 focus:ring-2 focus:ring-primary/50 focus:border-primary outline-none">
